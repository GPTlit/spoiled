...
function AuthPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      if (data.user) navigate({ to: (next as "/") || "/library" });
    });
  }, [navigate, next]);
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto flex max-w-md flex-col items-center px-4 py-16 sm:px-6">
        <span className="mb-4 inline-flex items-center gap-2 rounded-full border border-border/70 bg-card/60 px-3 py-1 text-[11px] uppercase tracking-[0.24em] text-muted-foreground">
          <Sparkles className="h-3 w-3 text-primary" /> Access your library
        </span>
        <h1 className="font-display text-4xl tracking-tight sm:text-5xl">
          {mode === "signup" ? "Create account" : "Welcome back"}
        </h1>
        <p className="mt-2 text-center text-sm text-muted-foreground">
          Save continuations, bookmark stories, and follow universes.
        </p>
        ...
        <div className="mt-8 w-full space-y-3">
          ...
          <form onSubmit={withEmail} className="space-y-2">
            ...
            <button
              type="submit"
              disabled={busy}
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-medium text-primary-foreground shadow-cinema transition hover:brightness-110 disabled:opacity-60"
            >
          ...
          <button
            onClick={() => navigate({ to: "/" })}
            className="w-full text-center text-[11px] uppercase tracking-[0.25em] text-muted-foreground hover:text-foreground"
          >
            Continue as guest
          </button>
        </div>
